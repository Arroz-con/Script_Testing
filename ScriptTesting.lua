...
getgenv().auto_trade_only_megas = true
...
local function autoTradeSemiAuto()
    ...
    local function autoTradeOnlyMegas()
        task.spawn(function()
            while task.wait(0.5) do
                if not getgenv().auto_trade_only_megas then return end
                pcall(function()
                    if not Player.PlayerGui.TradeApp.Frame.Visible then
                        ReplicatedStorage.API:FindFirstChild("TradeAPI/SendTradeRequest"):FireServer(selectedPlayer)
                    end
    ...
    Auto:AddToggle({
        Name = "Auto Trade Only Megas",
        Callback = function(value)
            getgenv().auto_trade_only_megas = value
            autoTradeOnlyMegas()
        end    
    })
